function [throughput, num_txs, num_txs_ch, n_agg_array, num_preventions, prevented_bw, tx_bw] = ...
        throuhgput_sim_full_buffer_hinder(occupancy_matrix, primary_ch, policy, N_agg_max, L_D, NUM_RFs)
    % Get throughput
    % Input:
    ...
    % - prevented_bw:  cumulative value of prevented BW [MHz]
    % - tx_bw: cumulative value of the bandwidth used during TX [MHz]
    ...
    %% for
    % States
    STATE_IDLE = -1;    % never this state if full-buffer considered
    STATE_DIFS = 0;
    STATE_BO = 1;
    STATE_TX = 2;
    ...
    num_txs_ch = zeros(NUM_RFs + 1,1); % From 0 to NUM_RFs possible channels to TX
    tx_bw = 0;
    % Buffer
    num_pkt_sent = 0;
    n_agg_array = [];
        ...
        while s < num_samples
            ...
            switch state
                ...
                case STATE_BO
                    ...
                    if ~sample_busy % Idle sample
                        if b < s_BO
                            ...
                            %fprintf('CB cont: %d, %d, %d, %d\n', s_FULL_TX, s_DATA, n_agg, bw)
                            if(s+s_FULL_TX <= num_samples)
                                ...
                                num_active_samples_in_tx = sum(sum(next_DATA_samples));
                                next_tx_free = logical(num_active_samples_in_tx==0);
                ...
                case STATE_TX
                    ...
                    % If TX not successful
                    if ~next_tx_free
                        num_preventions = num_preventions + 1;
                        prevented_bw = prevented_bw + num_active_samples_in_tx * BW_SC; 
                    end
    ...
    case POLICY_CBNONCONT
        ...
        %% Single-channel Gamble Saturated
        case POLICY_SC
            num_channels = 1;
            next_tx_free = true;
                        ...
                        if ~sample_busy % Idle sample
                            if b < s_BO
                                ...
                                if primary_ch >= 1 && primary_ch <= 8
                                    ...
                                    ch_range = 1:8;
                                    past_PIFS_samples = occupancy_matrix(s-s_PIFS:s,ch_range);
                                    num_active_samples_in_PIFS = sum(sum(past_PIFS_samples));
                                    access_free_8 = logical(num_active_samples_in_PIFS == 0);
                            ...
                            if b < s_BO
                                ...
                                if primary_ch >= 1 && primary_ch <= 8
                                    ...
                                    if ~access_free_8
                                        ...
                                        past_PIFS_samples = occupancy_matrix(s-s_PIFS:s,ch_range);
                                        num_active_samples_in_PIFS = sum(sum(past_PIFS_samples));
                                        access_free_4 = logical(num_active_samples_in_PIFS ==0);
                                        ...
                                        if ~access_free_4
                                            ...
                                            past_PIFS_samples = occupancy_matrix(s-s_PIFS:s,ch_range);
                                            num_active_samples_in_PIFS = sum(sum(past_PIFS_samples));
                                            access_free_2 = logical(num_active_samples_in_PIFS ==0);
                        ...
                        if ~sample_busy % Idle sample
                            if b < s_BO
                                ...
                                if primary_ch >= 1 && primary_ch <= 8
                                    ...
                                    past_PIFS_samples = occupancy_matrix(s-s_PIFS:s,ch_range);
                                    num_active_samples_in_PIFS = sum(sum(past_PIFS_samples));                                    
                                    access_free_second20MHz = logical(num_active_samples_in_PIFS ==0);
                                    ...
                                    past_PIFS_samples_40MHz_secondary_high = occupancy_matrix(s-s_PIFS:s,ch_range_40MHz_secondary_high);
                                    num_active_samples_in_PIFS = sum(sum(past_PIFS_samples_40MHz_secondary_high));    
                                    access_free_40MHz_secondary_high = logical(num_active_samples_in_PIFS ==0);
                                    ...
                                    past_PIFS_samples_40MHz_secondary_low = occupancy_matrix(s-s_PIFS:s,ch_range_40MHz_secondary_low);
                                    num_active_samples_in_PIFS = sum(sum(past_PIFS_samples_40MHz_secondary_low)); 
                                    access_free_40MHz_secondary_low = logical(num_active_samples_in_PIFS ==0);
                                    ...
                                    if length(ch_range) > 1
                                        ...
                                        past_PIFS_samples_80MHz_other = occupancy_matrix(s-s_PIFS:s,ch_range_80MHz_other);
                                        num_active_samples_in_PIFS = sum(sum(past_PIFS_samples_80MHz_other)); 
                                        access_free_80MHz_other = logical(num_active_samples_in_PIFS ==0);
